...
- 현재 요구사항·문서·백로그에 없는 미래용 기능, 설정, 플래그, 확장 포인트가 추가되었는지 확인한다.
- 구현체가 하나뿐인 interface, provider, strategy, factory, adapter, registry가 있는지 확인한다.
- 표준 라이브러리, 네이티브 API, 프레임워크 기본 기능, 기존 의존성으로 대체 가능한 새 코드·새 패키지가 있는지 확인한다.
- 동일해 보이는 코드라도 변경 이유가 다르면 무리하게 추상화하지 않았는지 확인한다.
- 태그:
...
- 체크:
  - [ ] 미래용 기능·설정·추상화가 없는가?
  - [ ] 표준/네이티브/기존 의존성으로 대체 가능한 새 구현이 없는가?
  - [ ] 단순화를 이유로 검증·보안·에러 처리·접근성·테스트 가능성을 제거하지 않았는가?

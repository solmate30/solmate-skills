---
name: verify-docs
description: 프로젝트 문서가 5단계 구조, 메타데이터, Related Documents, Backlog Context Lock, UI/Component Gate, Agent Harness Receipt를 준수하는지 검증합니다. 문서 생성·수정 후 또는 PR 전에 사용합니다.
---
...
9. Agent Harness Receipts
10. Rubric-First Writing
11. rules-product Gate Out 조건
...
- [ ] 각 작업 항목에 `Work Type`이 `code`, `deploy`, `docs`, `prototype` 중 하나로 기록되어 있는가?
...
`code`와 `deploy` 작업은 추가로 확인한다.

- [ ] Context Receipt의 상태가 PASS인가?
- [ ] `Required References Read`가 Related 필드의 모든 실제 링크를 포함하는가?
- [ ] Context Receipt에 Constraints와 Conflicts가 있는가?
- [ ] Change Receipt가 구현 범위와 기본 검사를 요약하는가?
- [ ] Done 또는 PR 준비 작업에 Verification Receipt가 있는가?
- [ ] Verification Receipt의 상태가 PASS이고 FAIL 결과가 없는가?
- [ ] 실행 명령, 결과, 미실행 검사 사유가 있는가?
- [ ] Detailed Evidence가 QA 문서 또는 PR을 링크하는가?

처음 5개 실제 작업의 `warning` 기간에는 Receipt 누락을 경고로 기록한다. `blocking` 전환 후에는 Context Receipt 누락이 구현을, Verification Receipt 누락이 Done·PR·배포를 차단한다.
...
### Step 9: Agent Harness Receipt 검사

대상 `code` 또는 `deploy` 작업에 대해 warning 기간에는 `--strict` 없이 실행해 발견 사항과 사용자 확인을 기록한다. blocking 전환 후에는 다음 명령을 실행한다.

```bash
npx solmate-skills preflight TASK-ID --strict
npx solmate-skills verify TASK-ID --strict
```

blocking 모드에서 두 명령 중 하나라도 non-zero이면 Agent Harness Gate는 Fail이다. warning 모드의 WARN을 PASS로 기록하면 안 된다. `docs`와 `prototype`은 `N/A - advisory work type`으로 기록한다.

### Step 10: Rubric-First Writing 검사
...
| Agent Harness Receipts | Pass / Fail / N/A | Context·Verification Receipt 및 상세 근거 |

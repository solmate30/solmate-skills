...
- **YAGNI**: 현재 문서·백로그·사용자 승인에 없는 미래용 기능, 설정, 추상화, 확장 포인트, provider/factory/interface를 미리 만들지 않는다.
- **KISS**: 승인된 요구사항을 만족하는 가장 단순한 해결을 우선한다. 기존 코드, 표준 라이브러리, 네이티브 API, 이미 설치된 의존성을 먼저 검토하고, 새 의존성·복잡한 패턴은 근거가 있을 때만 추가한다.
- **DRY**: 같은 지식과 같은 변경 이유를 가진 중복만 제거한다. 모양만 비슷하지만 서로 다른 이유로 바뀌는 코드는 억지로 추상화하지 않는다.
- **검토 순서**: (1) 안 만들어도 되는가? (2) 기존 코드로 해결되는가? (3) 표준/네이티브 기능으로 되는가? (4) 기존 의존성으로 되는가? (5) 최소 코드로 끝낼 수 있는가? 이 순서를 통과한 뒤에만 새 구현을 추가한다.

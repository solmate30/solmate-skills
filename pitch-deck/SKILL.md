---
name: pitch-deck
description: Create compelling pitch decks for investors, hackathons, and competitions. Supports three output modes — Markdown (content-first), Reveal.js HTML (browser presentation), or Stitch (visual design). Applies the 3 Investor Lenses and 6 Global Rubrics. Use when preparing for investor meetings, competition submissions, or demo days. Integrates with plan-docs Layer 1 documents.
---

# Pitch Deck Skill

This skill creates pitch decks optimized for investors, hackathon judges, and competition reviewers.
**Output mode는 상황에 따라 선택한다** — Markdown, Reveal.js HTML, Stitch 중 하나.

> This skill reads from `plan-docs` Layer 1 documents (VISION_CORE, LEAN_CANVAS, PRODUCT_SPECS) as source material. Run `plan-docs` first if those documents don't exist yet.
...
| **C. Stitch** | 시각적 완성도가 중요, 디자인이 필요한 발표 | Stitch 프로젝트 슬라이드 |
...
1. Stitch MCP prefix 탐색: `list_tools`로 prefix 확인
2. DESIGN.md가 있으면 읽어 디자인 시스템을 프롬프트에 포함
3. 슬라이드별 `generate_screen_from_text` 호출:
   - `deviceType`: `DESKTOP`
   - `prompt`: 슬라이드 내용 + DESIGN.md 디자인 시스템 블록
4. 생성된 슬라이드 HTML을 `pitch/slide-01.html` 형식으로 저장
5. 네비게이션 링크로 슬라이드 간 연결

**슬라이드별 프롬프트 구조:**
```
[Slide N: Slide Name] — [Product Name] Pitch Deck

DESIGN SYSTEM (REQUIRED):
[DESIGN.md Section 6 내용 복사]

Slide content:
- Headline: [핵심 메시지 1문장]
- Body: [슬라이드 본문 내용]
- Visual: [원하는 시각 요소 설명]
- Layout: Full-width slide, desktop 16:9
```

> DESIGN.md가 없으면 `design-md` 스킬로 먼저 생성하거나, 첫 슬라이드 생성 후 디자인 시스템을 추출한다.
...
- **plan-docs**: 피치덱의 원천 데이터 (VISION_CORE, LEAN_CANVAS, PRODUCT_SPECS)
- **manage-docs**: 공통 문서 규칙 및 365 Principle 정의
